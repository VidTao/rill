<script lang="ts">
  ...
  const TYPE_COLORS: Record<string, string> = {
    ...
    INT64: "text-blue-700",
    FLOAT64: "text-amber-700",
    FLOAT: "text-amber-700",
    BOOLEAN: "text-purple-700",
    TIMESTAMP: "text-red-700",
    DATE: "text-red-600",
    RECORD: "text-gray-500",
    ARRAY: "text-gray-500",
  };
</script>
...
<div class="flex h-full flex-col overflow-hidden">
  ...
  <div class="flex items-center justify-between border-b border-gray-200 px-4 py-2">
    <div class="flex items-center gap-3">
      ...
      <h1 class="text-lg font-semibold text-gray-800">Source Catalogs</h1>
      {#if !loading}
      ...
      <div class="mb-4">
        <div class="relative">
          <input
            ...
            class="w-full rounded border border-gray-300 px-3 py-2 pl-8 text-sm focus:border-blue-500 focus:outline-none"
          />
          <svg class="absolute left-2.5 top-2.5 h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="mb-6">
          ...
          <div class="space-y-1">
            {#each searchResults as result}
              <div class="flex items-center justify-between rounded bg-white px-3 py-2 text-xs border border-gray-100">
                <div class="flex items-center gap-2">
                  <span class="font-mono font-medium text-gray-800">{result.field}</span>
                  <span class="font-mono {TYPE_COLORS[result.type] ?? 'text-gray-500'}">{result.type}</span>
                </div>
                <div class="flex items-center gap-2">
                  <span class="text-gray-400">{result.label} / {result.stream}</span>
                  <button
